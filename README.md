...
- **OpenTelemetry Tracing:** Optional distributed tracing; auto-enabled via standard OTel env vars
...
│   ├── httpclient/           # OTel-instrumented http.Client helper
...
OpenTelemetry tracing is built in and disabled by default. It auto-enables when `OTEL_EXPORTER_OTLP_ENDPOINT` (or `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT`) is set. When disabled, only the W3C trace-context propagator is installed, so a `traceparent` header from an upstream component (e.g. a Kubernetes Gateway API) is still parsed and forwarded on outgoing calls.

- `OTEL_EXPORTER_OTLP_ENDPOINT`: collector endpoint (e.g. `http://otel-collector:4317`)
- `OTEL_EXPORTER_OTLP_PROTOCOL`: `grpc` (default) or `http/protobuf`
- `OTEL_SERVICE_NAME`: service name on emitted spans (defaults to `go-server-template`)
...
- Plus the usual `OTEL_EXPORTER_OTLP_HEADERS`, TLS, and signal-specific overrides

When a `traceparent` is present on the incoming request, this server's spans become children of the upstream trace — no double-trace.

**Instrumented out of the box:**

- HTTP server (chi): server spans named with the route template (e.g. `GET /users/{id}`)
- pgx (`internal/repository`): DB spans for queries and transactions
- Request logger emits `trace_id` / `span_id` fields for log↔trace correlation

For outgoing HTTP calls, use `httpclient.New()` from `internal/httpclient` to get a pre-instrumented `*http.Client` whose calls become children of the current request's span.

Checkout `otel-desktop-viewer` for local testing.
